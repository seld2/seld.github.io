...
function kp_dbCheck() {
	...
	if (get_option("kp_DbVersion", "") != $kp_DbVersion || $wpdb->get_var("SHOW TABLES LIKE '$visitTbl'") != $visitTbl) {
		kp_updateDatabase();
	}
...
function kp_updateDatabase() {
	...
	// Install the visits table
	if ($wpdb->get_var("SHOW TABLES LIKE '$visitTbl'") != $visitTbl) {
		$newVersion = "1.1";

...
function kp_determineIP() {
	...
	if (isset($_SERVER["HTTP_CLIENT_IP"]) && kp_checkIP($_SERVER["HTTP_CLIENT_IP"]) && $_SERVER["HTTP_CLIENT_IP"] != $serverIP) {
	...
	if (isset($_SERVER["HTTP_X_FORWARDED"]) && kp_checkIP($_SERVER["HTTP_X_FORWARDED"]) && $_SERVER["HTTP_X_FORWARDED"] != $serverIP) {
		return $_SERVER["HTTP_X_FORWARDED"];

	} else if (isset($_SERVER["HTTP_X_CLUSTER_CLIENT_IP"]) && kp_checkIP($_SERVER["HTTP_X_CLUSTER_CLIENT_IP"]) && $_SERVER["HTTP_X_CLUSTER_CLIENT_IP"] != $serverIP) {
		return $_SERVER["HTTP_X_CLUSTER_CLIENT_IP"];

	} else if (isset($_SERVER["HTTP_FORWARDED_FOR"]) && kp_checkIP($_SERVER["HTTP_FORWARDED_FOR"]) && $_SERVER["HTTP_FORWARDED_FOR"] != $serverIP) {
		return $_SERVER["HTTP_FORWARDED_FOR"];

	} else if (isset($_SERVER["HTTP_FORWARDED"]) && kp_checkIP($_SERVER["HTTP_FORWARDED"]) && $_SERVER["HTTP_FORWARDED"] != $serverIP) {
		return $_SERVER["HTTP_FORWARDED"];

	} else if (isset($_SERVER["REMOTE_ADDR"]) && kp_checkIP($_SERVER["REMOTE_ADDR"]) && $_SERVER["REMOTE_ADDR"] != $serverIP) {
 ...
 * @param int $randomness: Degree of randomness from 0 to 100. Lower means less random
 *
 * @return array<kp_recommendedPost>: An array of kp_recommendedPosts objects
 *
 * @since 1.2.5
 */
function kp_getRecommendedWP_Posts($numPostsToRecommend = -1, $ip = "", $ua = "", $recommendablePostTypes = array(), $randomness = 10) {
	$recommender = kp_runRecommender($numPostsToRecommend, $ip, $ua, $recommendablePostTypes, null, $randomness);
...
function kp_loadClasses() {
	...
	include_once(plugin_dir_path( __FILE__ ) . "classes/visits.php"); // The kp_visits class
}
...
function kp_renderWidget($numPostsToRecommend = -1, $recommendedPosts = array(), $template = "", $ip = "", $ua = "", $outputWidgetHtml = true, $widgetTitle = "", $post_style = "padding-top:10px;padding-bottom:10px;", $postimage_style = "display:inline;", $posttitle_style = "display:inline;", $postauthor_style = "display:inline;", $postdate_style = "display:inline;", $postteaser_style = "display:inline;", $before_widget = "", $after_widget = "", $before_title = "", $after_title = "", $alignment = "", $show_featuredimage = false, $show_posttitle = true, $show_postauthor = true, $show_postdate = true, $show_postteaser = false, $recommendablePostTypes = array(), $trackingCode = "", $testModeValue = null) {
	global $kp_templates, $kp_defaultAlignment, $kp_CodeVersion;
	...
	if (count($recommendedPosts) > 0){
		...
		// Render a comment to tell us where we are
		$widgetHtml = "<!-- Kindred Posts " . $kp_CodeVersion . " -->" . $widgetHtml;
	}
 ...
 * @since 1.2.5
 */
function kp_runRecommender($numPostsToRecommend = -1, $ip = "", $ua = "", $recommendablePostTypes = array(), $testModeValue = null, $randomness = 10) {
	...
	$recommender->randomness = $randomness;

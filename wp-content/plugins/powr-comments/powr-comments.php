<?php
     ...
     */
    /*
    Plugin Name: POWr Comments
    Plugin URI: https://www.powr.io/plugins/comments
    Description: Build a community of visitors or shoppers  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.3
    ...
    */
    ...
    class Powr_Comments extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_comments',
                             __( 'POWr Comments' ),
                             array( 'description' => __( 'Comments by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-comments' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Comments.
        </p>
        <p>
    ...
    function register_powr_comments() {
      register_widget( 'Powr_Comments' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_comments' );
    //Create short codes for adding plugins anywhere:
    function powr_comments_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-comments' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-comments' label='$label'></div>";
      }else{
      	"<div class='powr-comments'></div>";
      }
    }
    add_shortcode( 'powr-comments', 'powr_comments_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_comments_menu' );
    function powr_comments_menu() {
      add_menu_page( 'POWr Comments', 'POWr Comments', 'manage_options', 'powr-comments-settings', 'powr_comments_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_comments_options() {
      if ( !current_user_can( 'manage_options' ) )  {
        wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
      }
      echo '<iframe id="powr-comments" src="https://www.powr.io/wp-create/comments" frameborder="0" width="100%" height="600px" style="position:absolute; top:0; left:0;z-index: 100;"></iframe>';
      echo '<script>';
      echo 'var ht = window.innerHeight; if(document.getElementById("wpadminbar")){ht -= document.getElementById("wpadminbar").offsetHeight;}';
      echo 'var iframe = document.getElementById("powr-comments"); iframe.style.height = ht+"px"; iframe.height = ht;';
      echo '</script>';
    }
    //Redirecting to landing page when plugin is activated
    register_activation_hook(__FILE__, 'powr_comments_plugin_activate');
      add_action('admin_init', 'powr_comments_plugin_redirect');

      function powr_comments_plugin_activate() {
      add_option('powr_comments_plugin_do_activation_redirect', true);
      }

      function powr_comments_plugin_redirect() {
      if (get_option('powr_comments_plugin_do_activation_redirect', false)) {
          delete_option('powr_comments_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-comments-settings' );
          }
       }
      }
